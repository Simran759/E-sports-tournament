...
export default function TournamentDetails() {
  ...
  const { id } = useParams<{ id: string | string[] }>(); // Ensure `id` can be string or string[]
  ...
  const fetchTournamentDetails = async () => {
    try {
      // If `id` is an array, get the first value as the string ID
      ...
      if (typeof window !== "undefined") {
        try {
          // Save tournamentId to localStorage
          localStorage.setItem("TournamentId", tournamentId);
          console.log("Tournament ID stored in localStorage:", tournamentId);
        } catch (err) {
          console.error("Error saving TournamentId to localStorage:", err);
          setError("Failed to save tournament ID to localStorage.");
  ...
  const handleManageScore = (matchId: number) => {
    // Implement the logic to manage score for a match
              ...
              <table className="min-w-full table-auto border-separate border-spacing-2">
                <thead>
                  <tr className="bg-gray-600">
                    ...
                    <th className="p-2 text-lg font-semibold text-center text-white">Actions</th>
                  </tr>
                </thead>
                <tbody>
