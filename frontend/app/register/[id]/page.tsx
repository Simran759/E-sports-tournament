...
export default function RegisterTournament() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(`http://localhost:5000/player-tournaments/register/${id}`, payload);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-800 text-white">
      <h2 className="text-2xl mb-4">Register for Tournament</h2>
      <div className="bg-gray-900 p-6 rounded-lg shadow-lg w-96">
        ...
        <form onSubmit={handleRegister} className="flex flex-col gap-3">
          <input
            ...
            required
          />
          <button type="submit" className="bg-blue-500 px-4 py-2 rounded">Register</button>
        </form>
      </div>
    </div>
  );
