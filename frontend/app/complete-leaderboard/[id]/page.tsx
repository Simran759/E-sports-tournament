...
export default function TournamentDetails() {
  ...
  const handleRespectiveMatchesLeaderboard = () => {
    router.push(`/completed-matches/${id}`); // Navigate to respective matches leaderboard
  };

  const handleOverallLeaderboard = () => {
    router.push(`/completed-overall/${id}`); // Navigate to overall leaderboard
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-gray-700 text-white">
      ...
      <h6 className="text-1xl">Status: {status}</h6>
        ...
        <>
          <h2 className="text-xl mt-4 font-bold text-green-500">Tournament is completed! 🎮</h2>
          <p className="text-lg text-yellow-300">Check out the leaderedboard of results!</p>
